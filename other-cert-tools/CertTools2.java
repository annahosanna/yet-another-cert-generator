...
//   static Logger log = ProActiveLogger.getLogger(Loggers.SECURITY);
  ...
  public static Collection<X509Certificate> getCertsFromPEM(InputStream certstream)
    ...
    BufferedReader bufRdr = new BufferedReader(
      new InputStreamReader(certstream)
    );
    while (bufRdr.ready()) {
      ...
      String temp;
      while (
        ((temp = bufRdr.readLine()) != null) && !temp.equals(beginKey)
      ) continue;
      if (temp == null) {
      ...
      while (
        ((temp = bufRdr.readLine()) != null) && !temp.equals(endKey)
      ) opstr.print(temp);
      if (temp == null) {
        throw new IOException(
          "Error in " +
...
//        );
  ...
  public static String getCertificatePolicyId(X509Certificate cert, int pos)
    throws IOException {
    byte[] extvalue = cert.getExtensionValue(
      X509Extensions.CertificatePolicies.getId()
    );
    if (extvalue == null) {
      return null;
  ...
  public static URL getCrlDistributionPoint(X509Certificate certificate)
    ...
    try {
      ...
      for (int i = 0; i < distributionPoints.size(); i++) {
        ASN1Sequence distrPoint = (ASN1Sequence) distributionPoints.getObjectAt(
          i
        );
