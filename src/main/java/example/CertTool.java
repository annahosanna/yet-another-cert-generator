...
// https://raw.githubusercontent.com/ltrgithub/X3GestAA/refs/heads/master/cert_gen/java/src/com/sage/x3/syracuse/certtool/CertTool.java
...
public class CertTool {
    ...
    for (int i = result.length() - 1; i >= 0; i--) {
      ...
      if (c < ' ') {
        result.replace(
          i,
          i + 1,
          "\\x" + (c < 16 ? "0" : "") + Integer.toHexString(c)
        );
      }
    }
  ...
  private void longDn(String dn) throws IOException {
    String value = null;
    if ((value = findReplace(dn, "CN", null)) != null) wrapper.println(
      " Name: " + escapeChars(value)
    );
    if ((value = findReplace(dn, "OU", null)) != null) wrapper.println(
      " Organizational unit: " + escapeChars(value)
    );
    if ((value = findReplace(dn, "O", null)) != null) wrapper.println(
      " Organization: " + escapeChars(value)
    );
    if ((value = findReplace(dn, "L", null)) != null) wrapper.println(
      " City: " + escapeChars(value)
    );
    if ((value = findReplace(dn, "ST", null)) != null) wrapper.println(
      " State: " + escapeChars(value)
    );
    if ((value = findReplace(dn, "C", null)) != null) wrapper.println(
      " Country: " + escapeChars(value)
    );
  }

  /** Read certificate from file system and show its subject, issuer, not after date, not before date */
  private void showCertificateData(String name)
    throws CertToolException, IOException {
    String filename = getCertFileName(name);
    X509CertificateHolder holder;
    if (name == null && caCert != null) holder = caCert;
    else {
      ...
      try {
        ...
        } else throw new CertToolException("Not certificate");
      } finally {
        pemParser.close();
      }
    }
  ...
  private static String getKeyFileName(String name) {
    if (name == null) {
      // CA certificate
  ...
  private static String getCertFileName(String name) {
    if (name == null) {
      // CA certificate
  ...
  private static String getPublicKeyFileName(String name)
  ...
  static Date computeValidity(String days) {
    return new Date(
      System.currentTimeMillis() + 1000L * 86400L * Integer.parseInt(days)
    );
  }
  ...
  private void testInteractive(String errorMsg) throws CertToolException {
    if (!interactive) throw new CertToolException(errorMsg);
  }
  ...
  static KeyPair readKey(String name, char[] passphrase) throws IOException {
    ...
    try {
      ...
      if (p instanceof PEMEncryptedKeyPair) {
        keyPair = ((PEMEncryptedKeyPair) p).decryptKeyPair(
            jdb.build(passphrase)
          );
      } else if (p instanceof PEMKeyPair) {
        keyPair = (PEMKeyPair) p;
      } else throw new PEMException(
        "File " + filename + " does not contain private key"
      );
  ...
  String check(String input, Check test, boolean newName, boolean throwExc)
    ...
    switch (test) {
      case SERVER_NAME_NONE:
        if (input == null || input.length() == 0) {
          if (newName && caCert != null) exc =
            "CA certificate already generated";
          return null;
        }
      // no break!
      case SERVER_NAME:
        if (input == null || input.length() == 0) exc =
          "Server name must not be empty";
        else {
          input = input.toLowerCase();
          if (certNames.contains(input) ^ newName) return null;
          exc =
            "Server name " +
            input +
            (newName ? " already exists" : " does not exist");
        }
        break;
      case DN:
        if (input != null && input.length() >= 2) {
          return null;
        } else exc = "Entry must have at least length 2";
        break;
      case C:
        if (input != null && input.length() == 2) {
          return null;
        } else exc = "Entry must have length 2";
        break;
      case ACTION:
      ...
      case PORTZERO:
        ...
        if (choice == 0 && test == Check.PORTZERO) break;
    ...
    if (exc != null) {
      if (throwExc) throw new CertToolException(exc);
      else return exc;
    }
    return null;
  }
  ...
  String input(
    ...
    do {
      ...
      if (result.length() == 0 && defaultValue != null) result = defaultValue;
  ...
  static X509CertificateHolder readCertificate(String name) throws IOException {
    String filename = getCertFileName(name);
    wrapper.println("Read certificate " + filename);
    PEMParser pemParser = new PEMParser(new FileReader(filename));
    try {
      Object result = pemParser.readObject();
      if (!(result instanceof X509CertificateHolder)) throw new PEMException(
        "File " + filename + " does not contain certificate"
      );
   ...
   */
  static X509CertificateHolder generateCertificate(
    String issuerDn,
    KeyPair issuerPair,
    String subjectDn,
    ...
    ContentSigner cs = new JcaContentSignerBuilder("SHA256withRSA").build(
  ...
  private char[] readPrivateKey(String name, char[] passphrase, String message)
    throws CertToolException, IOException {
    while (true) {
      if (passphrase == null) {
        testInteractive(
          name == null
            ? "Passphrase for CA private key missing"
            : "Passphrase for private key missing"
        );
        passphrase = readPassphrase(message);
      }
      try {
        KeyPair kp = readKey(name, passphrase);
        if (name == null) caKey = kp;
        else serverKey = kp;
        return passphrase;
      } catch (EncryptionException ex) {
        if (interactive) {
          wrapper.println(
            "Error in decryption - probably incorrect passphrase"
          );
          passphrase = null;
        } else {
        ...
        throw new CertToolException(
          "Private key file missing: " + ex.getMessage()
        );
  ...
  static String findReplace(String dn, String id, String replacement) {
    ...
    if (index >= 0) {
      ...
      if (index2 >= 0) {
        if (replacement == null) return unescape(dn.substring(index, index2));
        else return (
          dn.substring(0, index) + escape(replacement) + dn.substring(index2)
        );
      } else {
        if (replacement == null) return unescape(dn.substring(index));
        else return dn.substring(0, index) + escape(replacement);
      }
    }
  ...
  CertTool() throws IOException, CertToolException {
    ...
    if (new File(getCertFileName(null)).exists()) {
      if (!new File(getKeyFileName(null)).exists()) {
        if (interactive) {
          wrapper.confirmMessage(
            "CA private key not available - generate new key"
          );
  ...
  boolean prepareAction()
    throws CertToolException, IOException, GeneralSecurityException, OperatorCreationException {
    // if no CA certificate present, generate this first
    if (caCert == null) {
      if (
        action != null && action != Action.CREATE
      ) throw new CertToolException("Have to create CA certificate first");
      if (name != null) throw new CertToolException(
    ...
    if (usePort && port < 0) throw new CertToolException(
      "Cannot transfer data when -notransfer option is set"
    );
    ...
    if (name == null && useName && interactive && caCert != null) {
      if (newName || !certNames.isEmpty()) {
        if (caAllowed) {
          name = input(
            "Name of server (empty input for CA)",
            Check.SERVER_NAME_NONE,
            newName,
            null
          );
        } else {
          name = input("Name of server", Check.SERVER_NAME, newName, null);
        }
      ...
      name,
      caAllowed ? Check.SERVER_NAME_NONE : Check.SERVER_NAME,
      newName,
      true
    );

    if (cert == null && name == null) cert = caCert;
    ...
    if (action == Action.RENEW_KEY) {
      if (name == null) {
        ...
        if (
          caKey == null &&
          new File(getKeyFileName(null)).exists() &&
          interactive
        ) {
          try {
            oldCaPass = readPrivateKey(
              null,
              oldCaPass,
              "Enter passphrase of private key of CA certificate: "
            );
          } catch (IOException ex) {
            wrapper.println(
              "Cannot read CA private key - please copy ca.cacrt manually to Syracuse servers " +
              ex.toString()
            );
            port = -1;
          }
    ...
    if (useDn && dn == null) {
      if (name == null) {
        if (dn == null) {
          ...
          if (interactive) {
            ...
            String o = input(
              "Organization",
              Check.DN,
              findReplace(dn, "O", null)
            );
            String ou = input(
              "Organizational unit",
              Check.DN,
              findReplace(dn, "OU", null)
            );
            if (cn == null) cn = input(
              "Name",
              Check.DN,
              findReplace(dn, "CN", null)
            );
            dn =
              "CN=" +
              escape(cn) +
            ...
            if (dn == null || cn == null) testInteractive("No subject given");
        ...
        dn = caCert.getSubject().toString();
        if (cn == null) {
          if (!newPass) cert = readCertificate(name);
          testInteractive("No subject name given");
          cn = input(
            "Server name for TCP connections",
            Check.DN,
            cert != null
              ? findReplace(cert.getSubject().toString(), "CN", null)
              : name
          );
        }
        dn = findReplace(dn, "CN", cn);
    ...
    if (validUntil == null && useDays) {
      ...
      if (caCert != null) validUntil = caCert.getNotAfter();
      if (name != null && certNames.contains(name)) {
        if (cert == null) cert = readCertificate(name);
      ...
      if (validUntil != null) {
        ...
        if (newDays.length() > 0) validUntil = computeValidity(newDays);
      } else {
        validUntil = computeValidity(
          input("Days of validity", Check.DAYS, null)
        );
      }
    }
    ...
    if (newPass) {
      if (pass == null && name != null) {
        ...
        if (interactive) {
          char[] confirm = readPassphrase(
            "Confirm passphrase of private key: "
          );
          if (!Arrays.equals(pass, confirm)) throw new CertToolException(
            "Passphrase does not match its confirmation"
          );
        }
      }
    ...
    if (newCaPass) {
      if (capass == null) {
        ...
        capass = readPassphrase(
          "Please enter passphrase for new private key of CA certificate: "
        );
        char[] confirm = readPassphrase(
          "Confirm passphrase of CA private key: "
        );
        if (!Arrays.equals(capass, confirm)) throw new CertToolException(
          "Passphrase does not match its confirmation"
        );
      }
      if (name == null) pass = capass;
    }
  ...
  private int askForTransfer(String name, boolean askPort)
    throws IOException, CertToolException {
    String port;
    ...
    if (askPort) {
      port = input(
        "Port of Syracuse server" +
        name +
        " if data should be transferred (no transfer for value 0)",
        Check.PORTZERO,
        false,
        "0"
      );
  ...
  void doAction()
    throws IOException, CertToolException, GeneralSecurityException, OperatorCreationException {
    String issuer;
    ...
    switch (action) {
      ...
      case RENEW_ALL_CERTS:
        ...
        caCertOld = caCert;
        caCert = generateCertificate(
          caCert.getSubject().toString(),
          caKey,
          cert.getSubject().toString(),
          extractPublicKey(caCert),
          validUntil
        );
        ...
        if (name == null) {
          ...
          if (!certNames.isEmpty()) { // also update other certificates
            ...
            for (String certName : certNames) {
              ...
              if (
                c.equals(findReplace(certDN, "C", null)) &&
                st.equals(findReplace(certDN, "ST", null)) &&
                l.equals(findReplace(certDN, "L", null)) &&
                o.equals(findReplace(certDN, "O", null)) &&
                ou.equals(findReplace(certDN, "OU", null))
              ) {
                certDN = findReplace(
                  issuer,
                  "CN",
                  findReplace(certDN, "CN", null)
                );
              }
              cert = generateCertificate(
                issuer,
      ...
      case DELETE:
        ...
        if (
          new File(getKeyFileName(name)).delete() &&
          new File(getCertFileName(name)).delete()
        ) {
          wrapper.println("Certificate and private key deleted for " + name);
          if (new File(getPublicKeyFileName(name)).delete()) {
            wrapper.println("Public key deleted for " + name);
          }
          certNames.remove(name);
        } else wrapper.println(
          "Could not delete certificate and private key for " + name
        );
        return;
      case SHOW:
   ...
   */
  private static void writePublic(String name, PublicKey publicKey)
    throws IOException, CertToolException {
  ...
  public static void main(String[] args) {
    try {
      ...
      ARGS: for (int i = 0; i < args.length; i++) {
        ...
        if (argument.length() > 0 && argument.charAt(0) == '-') {
          if (argument.equals("-help") || argument.equals("-?")) {
            // help text
            wrapper.println(
              "Invocation: java -jar certgen.jar [Task] [Parameters] [Name]"
            );
            for (Action act : Action.values()) {
              wrapper.println(
                "-" +
                act.name().replace('_', '-').toLowerCase() +
                "  " +
                act.getDescription()
              );
            }
            ...
            wrapper.println(
              "-pass <value>   Passphrase for server private key"
            );
            wrapper.println("-capass <value>  Passphrase for CA private key");
            wrapper.println(
              "-dn <value> Distinguished name of certificate subject"
            );
            wrapper.println(
              "-dn2 <values> Values for C, ST, L, O, OU, CN for name of certificate subject in this order"
            );
            wrapper.println(
              "-cn <value> Common name within distinguished name"
            );
            wrapper.println(
              "-days <value> Number of days of certificate validity"
            );
            wrapper.println(
              "-port <value> Transfer data to this port of a Syracuse server"
            );
            wrapper.println(
              "-wait <value> Timeout in seconds to connect to Syracuse server (default: " +
              DEFAULT_WAIT +
              ")"
            );
            wrapper.println(
              "-notransfer Do not ask for transfer of data to Syracuse servers"
            );
            wrapper.println("-batch  Do not allow input from console");
            wrapper.println(
              "-hex  Value of next parameter will be decoded from Hex string"
            );
            wrapper.println(
              "[Name] is the server name. If omitted, action is for the CA certificate"
            );
            return;
          }
          if (tool == null) tool = new CertTool();
          // parse option
          try { // action arguments
            Action temp = Action.valueOf(
              argument.substring(1).replace('-', '_').toUpperCase()
            );
            tool.action = temp;
            if (tool.actionGiven) throw new CertToolException(
              "Only one action can be specified"
            );
            else tool.actionGiven = true;
            continue;
            ...
            if (argument.equals("-notransfer")) {
              if (tool.port > 0) wrapper.println(
                "Warning: Ignore port because -notransfer option is set"
              );
              tool.port = -1;
              continue ARGS;
            }
            ...
            if ("-dn2".equals(argument)) {
              if (i >= args.length - 6) throw new CertToolException(
                "Not enough arguments for distinguished name parts"
              );
              String[] parts = new String[6];
              for (int j = 0; j < 6; j++) {
                if (hex) parts[j] = escape(hexdecode(args[++i]));
                else parts[j] = escape(args[++i]);
              }
              hex = false;
              String arg =
                "C=" +
            ...
            if ("-pass".equals(argument)) {
              if (arg == null) throw new CertToolException(
                "Missing passphrase"
              );
            ...
            if ("-port".equals(argument)) {
              ...
              if (tool.port == -1) wrapper.println(
                "Warning: Ignore port because -notransfer option is set"
              );
            ...
            if ("-attempts".equals(argument)) {
              if (arg == null) throw new CertToolException(
                "Missing number of attempts"
              );
              tool.check(arg, Check.DAYS, false, true);
              continue ARGS;
            }
            if ("-wait".equals(argument)) {
              if (arg == null) throw new CertToolException("Missing wait time");
              tool.check(arg, Check.DAYS, false, true);
            ...
            if ("-dn".equals(argument)) {
              ...
              try {
                ...
                throw new CertToolException(
                  "Distinguished name has wrong format: " + e.toString()
                );
              }
              tool.checkDn(arg);
              tool.dn = arg;
            ...
            if ("-days".equals(argument)) {
              if (arg == null) throw new CertToolException("Missing days");
              tool.check(arg, Check.DAYS, false, true);
        ...
        if (i == args.length - 1) {
          if (argument.length() > 0) tool.name = argument.toLowerCase();
        } else throw new CertToolException("Error in argument list");
      }
      if (tool == null) tool = new CertTool();
      ...
      do {
        try {
          if (tool.prepareAction()) break; // finish when 'END' has been chosen
          // do action
          tool.doAction();
      ...
      System.err.println(
        "Error in IO: " + ex.getClass().getSimpleName() + ": " + ex.getMessage()
      );
      System.exit(2);
    } catch (Exception ex) {
      ex.printStackTrace();
  ...
  private static String hexdecode(String arg)
  ...
  private void checkDn(String arg) throws CertToolException {
    ...
    if (list.size() > 0) throw new CertToolException(
      "Unknown attribute " + list.get(0) + " in distinguished name"
    );
  }
  ...
  private void checkValue(String attr, String val) throws CertToolException {
    ...
    if (error != null) throw new CertToolException(
      "Error in value '" +
      value +
      "' of attribute " +
      attr +
  ...
  private static void checkDn1(
    ArrayList<String> arg,
    String attribute,
    String longName
  ) throws CertToolException {
    if (!arg.remove(attribute)) throw new CertToolException(
      "Missing " + longName + " in distinguished name"
    );
    if (arg.remove(attribute)) throw new CertToolException(
      "Double " + longName + " in distinguished name"
    );
  }
}
...
enum Action {
  ...
  RENEW_KEY(
    "Generate new private key and create certificate with same subject as before"
  ),
...
class Exchange {
  ...
  Exchange() throws GeneralSecurityException {
    ...
    dhPubKey = ((DHPublicKey) diffieHellmanKeyPair.getPublic()).getY()
      .toByteArray();
  }
  ...
  public static void transfer(
    ...
    char[] pass,
    int port,
    X509CertificateHolder newCaCert
  ) throws GeneralSecurityException, IOException, CertToolException {
    instance(caCert, caKey);
    instance.transfer(crt, priv, pass, port, newCaCert);
  }

  private static void instance(X509CertificateHolder caCert, KeyPair caKey)
  ...
  void init(X509CertificateHolder caCert, KeyPair caKey)
    throws GeneralSecurityException {
    if (caCert == null) throw new RuntimeException(
      "Internal error: no CA certificate"
    );
    if (caKey == null) throw new RuntimeException("Internal error: no CA key");
    this.caCert = caCert;
    this.caKey = caKey;
    // make key out of certificate
    caCertIdentifier = caCert
  ...
  void transfer(
    ...
    try {
      ...
      CertTool.wrapper.println(
        "Transfer data to " + tcpHostname + ":" + port + " ..."
      );
      ...
      do {
        try {
          response = request(
            tcpHostname,
            port,
            "/nannyCommand/transferCertificate",
            protocol,
            challenge,
            ivDhPubKey,
            encryptedDhPubKey
      ...
      // check response
      if (response[0] != 0 && response[0] != 1) throw new CertToolException(
        "Wrong protocol"
      );
      if (response[0] == 1) {
        throw new CertToolException(
          "Error on Syracuse during key exchange: " +
          new String(response, 1, response.length - 1, UTF8)
        );
      }
      if (response.length < 65) throw new CertToolException(
        "Response too short"
      );
      // Message digest
      ...
      // test message digest
      for (int i = 0; i < digest.length; i++) if (
        digest[i] != response[i + 1]
      ) throw new CertToolException("Wrong CA certificate");
      ...
      byte[] decryptedPublicKey = cipher2.doFinal(
        response,
        105,
        response.length - 105
      );
      ...
      if (response[0] == 1) {
        throw new CertToolException(
          "Error on Syracuse during certificate transfer: " +
          new String(response, 1, response.length - 1, UTF8)
        );
      }
      ...
      if (certTool.interactive) {
        CertTool.wrapper.confirmMessage(
          "Cannot connect to server: " + e.toString()
        );
      } else throw e;
      return;
    } catch (Exception e) {
