...
// https://raw.githubusercontent.com/wso2/carbon-device-mgt/refs/heads/master/components/certificate-mgt/org.wso2.carbon.certificate.mgt.core/src/main/java/org/wso2/carbon/certificate/mgt/core/impl/CertificateGenerator.java
...
public class CertificateGenerator {
  ...
  public static void extractCertificateDetails(
    ...
    try {
      if (certificateBytes != null) {
        java.security.cert.Certificate x509Certificate =
          (java.security.cert.Certificate) Serializer.deserialize(
            certificateBytes
          );
        if (x509Certificate instanceof X509Certificate) {
          ...
          certificateResponse.setNotBefore(
            certificate.getNotBefore().getTime()
          );
          certificateResponse.setCertificateserial(
      ...
      String errorMsg =
        "Error while during deserialization of the certificate.";
      throw new CertificateManagementDAOException(errorMsg, e);
    }
  }

  public List<X509Certificate> getRootCertificates(byte[] ca, byte[] ra)
    ...
    try {
      ...
      X509Certificate caCert =
        (X509Certificate) certificateFactory.generateCertificate(caInputStream);
      X509Certificate raCert =
        (X509Certificate) certificateFactory.generateCertificate(raInputStream);
  ...
  public X509Certificate generateX509Certificate() throws KeystoreException {
    ...
    try {
      KeyPairGenerator keyPairGenerator = KeyPairGenerator.getInstance(
        CertificateManagementConstants.RSA,
        CertificateManagementConstants.PROVIDER
      );
      keyPairGenerator.initialize(
        CertificateManagementConstants.RSA_KEY_LENGTH,
        new SecureRandom()
      );
      KeyPair pair = keyPairGenerator.generateKeyPair();
      X500Principal principal = new X500Principal(
        CertificateManagementConstants.DEFAULT_PRINCIPAL
      );

      X509v3CertificateBuilder certificateBuilder =
        new JcaX509v3CertificateBuilder(
          principal,
          CommonUtil.generateSerialNumber(),
          validityBeginDate,
          validityEndDate,
          principal,
          pair.getPublic()
      ...
      ContentSigner contentSigner = new JcaContentSignerBuilder(
        CertificateManagementConstants.SHA256_RSA
      )
      ...
      List<org.wso2.carbon.certificate.mgt.core.bean.Certificate> certificates =
        new ArrayList<>();
      ...
      certificateToStore.setTenantId(
      ...
      String errorMsg =
        "Issue in operator creation when generating certificate";
      throw new KeystoreException(errorMsg, e);
      ...
      String errorMsg =
        "Certificate issue occurred when generating certificate";
      throw new KeystoreException(errorMsg, e);
      ...
      String errorMsg =
        "Signature related issue occurred when generating certificate";
      throw new KeystoreException(errorMsg, e);
    }
  }

  public byte[] getPKIMessage(InputStream inputStream)
    throws KeystoreException {
    try {
      ...
      X509Certificate certRA =
        (X509Certificate) keyStoreReader.getRACertificate();
      X509Certificate certCA =
        (X509Certificate) keyStoreReader.getCACertificate();
      ...
      PkcsPkiEnvelopeDecoder envelopeDecoder = new PkcsPkiEnvelopeDecoder(
        certRA,
        privateKeyRA
      );
      PkiMessageDecoder messageDecoder = new PkiMessageDecoder(
        reqCert,
        envelopeDecoder
      );
      ...
      PKCS10CertificationRequest certRequest =
        (PKCS10CertificationRequest) msgData;
      ...
      String errorMsg =
        "Certificate issue occurred when generating getPKIMessage";
      throw new KeystoreException(errorMsg, e);
    } catch (MessageEncodingException e) {
      String errorMsg =
        "Message encoding issue occurred when generating getPKIMessage";
      throw new KeystoreException(errorMsg, e);
    } catch (IOException e) {
      String errorMsg =
        "Input output issue occurred when generating getPKIMessage";
      throw new KeystoreException(errorMsg, e);
    } catch (MessageDecodingException e) {
      String errorMsg =
        "Message decoding issue occurred when generating getPKIMessage";
      throw new KeystoreException(errorMsg, e);
  ...
  public boolean verifySignature(String headerSignature)
    throws KeystoreException {
    Certificate certificate = extractCertificateFromSignature(headerSignature);
    return (certificate != null);
  }

  public CertificateResponse verifyPEMSignature(
    X509Certificate requestCertificate
  ) throws KeystoreException {
    if (requestCertificate == null) {
      throw new IllegalArgumentException(
        "Certificate of which the signature needs to be validated cannot " +
        "be null"
      );
    }
    KeyStoreReader keyStoreReader = new KeyStoreReader();
  ...
  public CertificateResponse verifyCertificateDN(String distinguishedName)
    throws KeystoreException {
      ...
      if (distinguishedName.contains("/CN=")) {
        ...
        for (Rdn relativeDistinguishedNames : ldapName.getRdns()) {
          if (relativeDistinguishedNames.getType().equalsIgnoreCase("CN")) {
            lookUpCertificate = keyStoreReader.getCertificateBySerial(
              String.valueOf(relativeDistinguishedNames.getValue())
            );
            break;
          }
        }
  ...
  public X509Certificate pemToX509Certificate(String pem)
    ...
    try {
      inputStream = new ByteArrayInputStream(
        Base64.decodeBase64(pem.getBytes())
      );
      CertificateFactory cf = CertificateFactory.getInstance("X.509");
      x509Certificate = (X509Certificate) cf.generateCertificate(inputStream);
    } catch (CertificateException e) {
      String errorMsg =
        "Certificate issue occurred when generating converting PEM to x509Certificate";
  ...
  public X509Certificate extractCertificateFromSignature(
    ...
    try {
      ...
      CMSSignedData signedData = new CMSSignedData(
        Base64.decodeBase64(headerSignature.getBytes())
      );
      Store reqStore = signedData.getCertificates();
      @SuppressWarnings("unchecked")
      Collection<X509CertificateHolder> reqCerts = reqStore.getMatches(null);
      ...
      String errorMsg =
        "CertificateException when decoding certificate signature";
      throw new KeystoreException(errorMsg, e);
    }
  ...
  public X509Certificate generateCertificateFromCSR(
    ...
    if (request.getSubject().getRDNs(BCStyle.UNIQUE_IDENTIFIER).length != 0) {
      ...
      certUniqueIdentifier = BigInteger.valueOf(
        certUniqueIdRDN[0].getFirst().getValue().toString().hashCode()
      );
    ...
    X509v3CertificateBuilder certificateBuilder = new X509v3CertificateBuilder(
      new X500Name(issueSubject),
      certUniqueIdentifier,
      validityBeginDate,
      validityEndDate,
      certSubject,
    ...
    try {
      ...
      String errorMsg =
        "Certificate Input output issue occurred when generating generateCertificateFromCSR";
      throw new KeystoreException(errorMsg, e);
    } catch (OperatorCreationException e) {
      String errorMsg =
        "Operator creation issue occurred when generating generateCertificateFromCSR";
      throw new KeystoreException(errorMsg, e);
    } catch (CertificateException e) {
      String errorMsg =
        "Certificate issue occurred when generating generateCertificateFromCSR";
      throw new KeystoreException(errorMsg, e);
    }
  ...
  private ASN1Encodable getChallengePassword(Attribute[] attributes) {
    for (Attribute attribute : attributes) {
      if (
        PKCSObjectIdentifiers.pkcs_9_at_challengePassword.equals(
          attribute.getAttrType()
        )
      ) {
        if (
          attribute.getAttrValues() != null &&
          attribute.getAttrValues().size() > 0
        ) {
  ...
  private CMSSignedData getMessageData(final List<X509Certificate> certs)
    ...
    try {
      ...
      String errorMsg =
        "Certificate encoding issue occurred when generating getMessageData";
      throw new KeystoreException(errorMsg, e);
    } catch (CMSException e) {
      String errorMsg =
        "Message decoding issue occurred when generating getMessageData";
      throw new KeystoreException(errorMsg, e);
    }
  }
  ...
  public void saveCertInKeyStore(
    List<org.wso2.carbon.certificate.mgt.core.bean.Certificate> certificate
  ) throws KeystoreException {
    if (certificate == null) {
      return;
    }
    ...
    try {
      CertificateDAO certificateDAO =
        CertificateManagementDAOFactory.getCertificateDAO();
   ...
   * @throws KeystoreException
   */
  public X509Certificate getSignedCertificateFromCSR(
    String binarySecurityToken
  ) throws KeystoreException {
    byte[] byteArrayBst = DatatypeConverter.parseBase64Binary(
      binarySecurityToken
    );
    ...
    return generateCertificateFromCSR(
      privateKeyCA,
